          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="sale in spuSaleAttrList" :key="sale.id">
                ...
                <!-- 售卖属性的属性值 -->
                <dd v-for="valuelist in sale.spuSaleAttrValueList" 
                :key="valuelist.id" changepirce="0" 
                :class="{active:valuelist.isChecked ==1}"
                @click="changeActive(valuelist,sale.spuSaleAttrValueList)"
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" type="" @change='numberChange' v-model.number="number" class="itxt">
                <a href="javascript:" @click="number++" class="plus">+</a>
                <a href="javascript:" @click = "number>1?number--:number = 1" class="mins">-</a>
              </div>
              <div class="add">
                <a @click="addCar" href="javascript:">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    // 挂载派发action获取点击的商品信息
    mounted() {
      this.$store.dispatch('detail/proDetail',this.$route.params.proId)
    },
    computed: {
      ...mapGetters('detail',{
        ...
        skuImageList(){
          // console.log(this.skuInfo.skuImageList,'222222');
          return this.skuInfo.skuImageList||[]
        }
    },
    methods: {
      changeActive(salValue,arr){
        // console.log(a,b)
        arr.forEach(list => {  // 遍历所有的属性，全部改为未选中，将选中的那个value改为选中
          list.isChecked = 0
        });
        salValue.isChecked =1
      },
      // 添加购物车数量改变时触发回调
      numberChange(e){
        let inp = e.target.value*1
        if(isNaN(inp)) console.log("请输入合法的数字");
        else
        this.number =  parseInt(Math.abs(e.target.value))
